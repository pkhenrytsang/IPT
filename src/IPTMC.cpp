/*
	...
	3. Replacing trapezoidl rule with Monte Carlo
	4. by default, the output is stored in "Gf.out" and "Sig.out" unless debug mode is enabled, which output and input will be saved to output.res and input.res respectively
	5. PARAMS file must be supplied completely and accurately - else program will abort. For some fields "default" is acceptable.
	6. Internally, SIAM solver class is "standalone" in the sense that it is the only class needed to run the impurity solver code.
*/
...
int main(int argc, char* argv[])
{
  ...
  params.ReadParam(CheckSpectralWeight,"'CheckSpectralWeight'"); //default false
	...
	if (rank != 0){
		...
		L = new double[fitorder];
		R = new double[fitorder];
		//printf("Allocation memory for rank %d\n",rank);
	}
	...
	MPI_Bcast(&AmoebaMaxIts,1,MPI_INT,0,MPI_COMM_WORLD);
	MPI_Bcast(&AmoebaScanStart,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Bcast(&AmoebaScanEnd,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Bcast(&AmoebaScanStep,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Bcast(&AmoebaForceScanAndPrintOut,1,MPI_C_BOOL,0,MPI_COMM_WORLD);
  ...
  struct siammcparams solverparams;
  ...
  solverparams.verbose = verbose;
	solverparams.AmoebaMaxIts = AmoebaMaxIts;
	solverparams.AmoebaScanStart = AmoebaScanStart;
	solverparams.AmoebaScanEnd = AmoebaScanEnd;
	solverparams.AmoebaScanStep = AmoebaScanStep;
	solverparams.AmoebaForceScanAndPrintOut = AmoebaForceScanAndPrintOut;
	
	//printf("fitorder of rank %d : %d\n",rank,fitorder);
	//printf("M of rank %d : %d\n",rank,M);
  MPI_Barrier(MPI_COMM_WORLD);
	//Initialize the solver using supplied arguments
	SIAMMC Solver(omega,N,&solverparams);
